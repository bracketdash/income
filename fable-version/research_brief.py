"""
Batch headline pull + red-flag keyword scan for candidates (Step 3).

Front-loads the raw material for every candidate in one pass so the cost of
checking a name is never a reason to skip it. It is a starting point, not a
verdict, and it has two known weaknesses that the process works around:
  ...
  2. The red-flag regexes match keywords, not meaning. Across the first
     cycles "merger" fired on a completed divestiture and "guidance" fired
     on a guidance RAISE. Treat a hit as "go read it", never as a finding.
...
Usage:
    ...
    python research_brief.py --tickers BX,DNOW
    python research_brief.py --days 21
"""
...
def main():
    ap = argparse.ArgumentParser(description="Batch research brief for candidates")
    ...
    if args.tickers:
        ...
        if SHORTLIST.exists():
            meta = pd.read_csv(SHORTLIST).set_index("Symbol").to_dict("index")
    else:
    ...
    print("dates, index changes. Step 3 requires one events check per finalist.")
