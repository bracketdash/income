"""
Grade closed-out batches and report what the whole log says (Step 0).
...
After grading, this prints two layers:
  1. The batch: per-name results, portfolio vs equal-weight, and the
     batch's return against SPY over the same window -- because a -2%
     week in a -2% tape and a -2% week in a +1% tape are different facts.
  2. The POOLED record across every graded batch: conviction calibration,
     ...
     are ~12 names; the pooled view is where a pattern becomes visible and
     where the process rules about small samples apply.

Usage:
    python review_picks.py            # grade what is due, then pooled summary
    python review_picks.py --pooled   # pooled summary only, no fetching
"""
import argparse
import sys
import time
...
def summarize_batches(rows):
    ...
    for batch, g in rows.groupby("BatchDate"):
        ...
        research_funnel(batch, g["Ticker"].tolist(), pd.to_datetime(g["WindowEndDate"].iloc[0]))


def research_funnel(batch, picks, window_end):
    """Did research selection beat the raw screen? Equal-weight open->close
    for: the whole archived shortlist, its top 30 by score, the names
    researched and rejected, the names never researched, and the picks.
    If the picks trail the never-researched remainder, the funnel started
    in the wrong place; if they trail the rejected set, selection hurt."""
    arch = HERE / ".cache" / "shortlists" / f"shortlist_{batch}.csv"
    if not arch.exists():
        return
    sl = pd.read_csv(arch)
    syms = sl["Symbol"].tolist()
    try:
    ...
    if rl.exists():
        r = pd.read_csv(rl)
        researched = set(r[r["BatchDate"] == batch]["Ticker"])
    picks = set(picks)
...
def pooled(df):
    ...
    if len(g) < 40 or nb < 5:
        print(f"(Below the 5-batch / 40-pick bar for changing any constant. Read for direction only.)")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Grade closed-out batches; pooled summary")
