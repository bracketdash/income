...
whole universe, which is ~40 weekly rebalances x ~1,500 liquid names -- the
right place to ask "does this flag predict next week?" before changing a
weight, and the reason PROCESS.md forbids changing score weights without
running this first.
...
Known limits, so the numbers are read with the right scepticism:
  ...
  - Nothing here is a transaction-cost or slippage model. Open->close is
    the grading convention, not a fill.

Usage:
    python backtest_screen.py
    python backtest_screen.py --weights 0.40,0.25,0.15,0.20   # test other weights
    python backtest_screen.py --top 40
"""
...
def load_panels():
    paths = sorted(glob.glob(str(HERE / ".cache" / "prices_*.pkl")), reverse=True)
    if not paths:
        raise SystemExit("No price cache -- run swing_screen.py first.")
...
def main():
    ...
    for i in range(len(weeks) - 1):
        ...
        r = dict(week=nf.date(), n=len(d), univ=d.fwd.mean(),
    ...
    R.to_csv(HERE / "backtest_last.csv", index=False)
