"""
Price-structure check for candidates (Step 3 of PROCESS.md).

The screener's flags say "did this pattern roughly fire". This reads how
the chart is actually behaving over the horizon that matters for a one-week
hold, and raises the conditions that most often make a written thesis wrong:

  EXTENDED   >10% above the 20d SMA. NOT a sell signal on its own -- across
             40 backtested weeks the 10-20%-extended bucket was the BEST
             performing extension bucket (+0.71%/wk vs +0.23% for 0-5%).
             It is a *labelling* constraint: a name this far above its
             average cannot honestly be called a pullback, and its expected
             move is usually wider than trailing ATR says.
  BELOW20    under its 20d SMA. Fatal to any pullback-in-an-uptrend thesis.
             The 2026-08-31 batch called AXON a pullback while it sat below
             its 20d; it lost 13%.
  DISTRIB    down sessions carrying more volume than up sessions over the
             last 10. Supply in control while price still looks fine. The
             same batch called U "not distribution" at 0.50; it lost 4%.
  GAPFADE    a recent gap that price has given back. Post-earnings drift
             works when the gap holds.
  VOLEXP     ATR expanding >30% vs 20 sessions ago: it will move more than
             the sizer's ATR14 assumes.

A flag is a question the thesis has to answer honestly, not a veto. What
is not acceptable is a flag going unmentioned in a thesis it contradicts.

Reads the price cache swing_screen.py wrote, so it sees the same bars.

Usage:
    python technicals.py                      # top 30 of shortlist.csv
    python technicals.py --tickers BX,AXON
    python technicals.py --short-interest     # one live call per name
"""
...
def main():
    ...
    if by_flag:
        print("Flagged -- each needs an honest answer in its thesis (not a veto, not a sentence either):")
