"""
Turn researched finalists into a ranked, risk-weighted allocation table.

Step 4 of PROCESS.md. Research supplies a conviction score per name; this
does every piece of arithmetic after that, so sizing involves no judgment.

Sizing: weight ~ conviction^1.5 / expected-move, capped per name and per
sector. Inverse-volatility because an equal-dollar book across names with
very different volatility is a concentrated bet on the wild ones wearing a
diversified costume. This has beaten equal-weight of the same names in
every graded batch so far (+1.43, +0.29, +0.22 pts) -- though note all
three were down weeks, where underweighting volatile names wins
mechanically. Keep watching it in up weeks.

Cash: two bounds, the tighter one wins.
  ...
  Bound (a) alone was the original design and produced 0% cash in regime 2
  twice in three batches -- a book of clean, low-ATR names measures under
  even a low target, and trailing ATR understates an event week anyway.
  Bound (b) is what makes the regime score actually brake.

What is and is not forecastable: expected MOVE (dispersion) is -- vol
clusters. Expected RETURN at a one-week horizon is not, to any useful
precision. Conviction is ordinal. The arithmetic does not add accuracy the
judgment did not have.

Usage:
    python rank_and_size.py candidates.json --regime 3 --out picks.json
    python rank_and_size.py --show-rubric
...
MIN_CONVICTION = 3       # 2s were the worst pooled bucket; excluded unless --allow-2
...
def build(rows, regime, deploy=None, as_of=None, sessions=DEFAULT_WINDOW_DAYS, allow_2=False):
    ...
    for _, r in df[df["ExpMovePct"] < MIN_EXP_MOVE_PCT].iterrows():
        print(f"Warning: {r['Ticker']} expected range only {r['ExpMovePct']:.1f}% -- sized as "
              f"{MIN_EXP_MOVE_PCT:.1f}%; it may not move enough to earn its slot.")
...
def main():
    ap = argparse.ArgumentParser(description="Rank and risk-weight researched candidates")
    ...
    if args.show_rubric:
        ...
        return
    if not args.input or args.regime is None:
        sys.exit("usage: rank_and_size.py candidates.json --regime N [--out picks.json]")
    ...
    if args.out:
        ...
        Path(args.out).write_text(json.dumps(payload.to_dict(orient="records"), indent=2), encoding="utf-8")
        print(f"\nWrote {args.out}")
