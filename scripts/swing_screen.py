"""
Step 1 -- screen the NYSE/NASDAQ universe to a ranked shortlist + regime snapshot.

Narrows ~5,000 common stocks to the 60 best-scored names and writes them to
runs/<batch>/shortlist.csv, alongside runs/<batch>/regime.txt. Every later
step reads the price cache this script writes, so they all see the same bars.

What the score is built on (re-test with backtest_screen.py before changing):
  Measured over 40 weekly rebalances as next-week open->close return against
  the liquid non-downtrend universe --
    Breakout flag        +1.03 pts/wk, t = +2.68, positive in both halves
    RS_20d top quintile  -0.00 pts/wk, t = -0.01  (flat across all quintiles)
    Pullback flag        -0.53 pts/wk, t = -1.54, negative in both halves
    GapEvent flag        +0.16 pts/wk, t = +0.40
  So Breakout carries the score, RS_20d only breaks ties, GapEvent is a
  small nudge, and Pullback has no score weight. The Pullback flag is still
  computed and printed because it is a useful *label* in Step 3 -- it is
  just not a reason to rank a name higher.

Usage:
    python scripts/swing_screen.py
    python scripts/swing_screen.py --refresh-universe
"""
...
from paths import CACHE_DIR, REGIME, SHORTLIST, run_dir
...
def fetch_universe(force_refresh=False):
    ...
    delistings, which Step 3 checks against the finalists' acquirers."""
    ...
    # 1-5 capital letters only; drop 5-letter symbols ending in W (warrants)
...
def compute_regime(frames, breadth_symbols):
    ...
    lines += ["", "Macro calendar (FOMC / CPI / PCE / jobs) and rate direction are NOT",
              "pulled here -- confirm by web search before scoring the regime (Step 2)."]
    return "\n".join(lines)
...
def compute_metrics(symbol, df, spy_df):
    ...
    return {
        ...
        # A value near zero on a name that just jumped means a pinned cash deal.
...
def _fetch_one(symbol):
    """Sector, next/last earnings date and ex-dividend date for one symbol,
    with rate-limit backoff. Returns (symbol, record_or_None)."""
    for attempt in range(5):
        try:
            ...
            if "Rate limit" in str(e) or "Too Many Requests" in str(e):
                ...
                continue
            return symbol, {"sector": "", "next_earnings": "", "last_earnings": "", "ex_div": "",
                            "fetched": datetime.now().strftime("%Y-%m-%d"), "error": str(e)[:80]}
    return symbol, None


def enrich_top(df):
    """Sector, earnings dates and ex-dividend date for the shortlist. Cached
    per symbol for ENRICH_MAX_AGE_DAYS and fetched in parallel.

    A blank NextEarnings means the lookup FAILED, not that nothing is
    scheduled. The lookup only knows quarterly dates: companies that report
    monthly, investor days and FDA dates are invisible to it. Step 3's
    by-hand events check exists for exactly that reason."""
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Step 1: screen the universe to a ranked shortlist")
    ...
    (out_dir / REGIME).write_text(regime_text, encoding="utf-8")
    print("\n--- REGIME ---\n" + regime_text + "\n--------------\n")
    ...
    # Tiebreak by VolRatio: volume confirmation is what carries the breakout signal.
    ...
    shortlist.to_csv(out_dir / SHORTLIST, index=False)
    log(f"Wrote {len(shortlist)} candidates to {out_dir / SHORTLIST} "
